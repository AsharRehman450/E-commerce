...
const fetchGymProducts = async () => {
  const res = await axios.get(
    "http://localhost:5000/api/products?category=Gym"
  );
...
const Gym = () => {
  const {
    ...
    queryKey: ["gym"],
    queryFn: fetchGymProducts,
    staleTime: 3 * 60 * 1000,
  });

  if (isLoading) return <p className="text-center py-10">Loading...</p>;
  if (isError) {
    toast.error("Failed to load gym products.", { autoClose: 2000 });
    return (
      <p className="text-center py-10 text-red-500">
        Failed to load gym products
      </p>
    );
  }
  ...
  return (
    <>
      <div className="px-4 py-8">
        <h1 className="text-4xl font-bold text-center mb-8 mt-8">Gym Wear</h1>
          ...
          {products.map((product) => (
            <Link to={`/product/${product._id}`} key={product._id}>
              <div className="text-center w-[200px] hover:scale-105 transition-transform duration-300">
                ...
                <p className="font-semibold mt-2 text-left">{product.title}</p>
