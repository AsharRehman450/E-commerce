...
const fetchReviews = async (productId) => {
  const { data } = await axios.get(`http://localhost:5000/api/review/product/${productId}`);
  return data;
};
...
const ProductTabs = ({ productId }) => {
  ...
  const reviewMutation = useMutation({
    mutationFn: (payload) => {
      if (payload.editingId) {
        return axios.put(`http://localhost:5000/api/review/${payload.editingId}`, payload.review);
      }
      return axios.post(`http://localhost:5000/api/review`, {
        ...payload.review,
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => axios.delete(`http://localhost:5000/api/review/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries(["reviews", productId]);
    },
  });
