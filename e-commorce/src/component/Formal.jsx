...
const fetchFormalProducts = async () => {
  const res = await axios.get(
    "http://localhost:5000/api/products?category=Formal"
  );
...
const Formal = () => {
  const {
    ...
    queryKey: ["formal"],
    queryFn: fetchFormalProducts,
    staleTime: 3 * 60 * 1000,
  });

  if (isLoading) return <p className="text-center py-10">Loading...</p>;
  if (isError) {
    toast.error("Failed to load formals products.", { autoClose: 2000 });
    return (
      <p className="text-center py-10 text-red-500">
        Failed to load formals products
      </p>
    );
  }
